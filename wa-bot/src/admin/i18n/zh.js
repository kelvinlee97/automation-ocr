/* eslint-disable quote-props */
"use strict";

module.exports = {
  "admin_panel": "ClaimFlow 管理后台",
  "connected": "已连接",
  "disconnected": "未连接",
  "receipt_review": "收据审核",
  "download_excel": "下载 Excel",
  "user_management": "用户管理",
  "switch_to_light": "切换到浅色模式",
  "switch_to_dark": "切换到深色模式",
  "logout": "退出",
  "switch_language": "切换语言",
  "lang_zh": "中文",
  "lang_en": "EN",
  "page_previous": "上一页",
  "page_next": "下一页",
  "page_info": "第 {current} 页，共 {total} 页",
  "admin_login": "管理后台登录",
  "username": "用户名",
  "password": "密码",
  "login_btn": "登 录",
  "init_admin": "初始化管理后台",
  "first_time_setup": "首次使用，请创建管理员账号",
  "username_hint": "用户名（至少 3 位，字母/数字/下划线）",
  "password_hint": "密码（至少 8 位）",
  "confirm_password": "确认密码",
  "create_admin": "创建管理员账号",
  "setup_hint": "此页面只在尚无账户时出现，创建后自动消失",
  "manage_users": "用户管理",
  "new_user": "新建用户",
  "created_at": "创建时间",
  "actions": "操作",
  "no_users": "暂无用户",
  "current": "当前",
  "cannot_delete_self": "不能删除当前登录账户",
  "delete": "删除",
  "reset_password": "重置密码",
  "confirm_delete": "确认删除用户 {username}？",
  "prompt_new_password": "请输入 {username} 的新密码（至少 8 位）：",
  "password_min_8": "密码至少 8 位",
  "create_user_title": "新建用户",
  "create_user_btn": "创建用户",
  "cancel": "取消",
  "user_created": "用户创建成功",
  "password_reset_ok": "密码重置成功",
  "user_deleted": "用户已删除",
  "receipt_audit": "收据审核",
  "group_receipts": "张收据",
  "collapse": "折叠",
  "expand": "展开",
  "no_receipts": "暂无收据记录",
  "total": "总计",
  "pending": "待审核",
  "ai_extracted": "AI已提取",
  "approved": "已通过",
  "rejected": "已拒绝",
  "search_placeholder": "搜索手机号/IC号...",
  "all_statuses": "全部状态",
  "result_count": "共 {count} 条",
  "col_num": "#",
  "col_submit_time": "提交时间",
  "col_phone": "手机号",
  "col_ic": "身份证号",
  "col_receipt_img": "收据图片",
  "col_status": "状态",
  "col_ai_result": "AI 提取结果",
  "col_actions": "操作 / 记录",
  "receipt_large": "收据大图",
  "scan_login": "扫码登录",
  "connect_whatsapp": "连接 WhatsApp",
  "scan_qr": "扫描二维码",
  "pairing_code": "配对码登录",
  "qr_hint": "用 WhatsApp 扫码后页面自动跳转",
  "qr_refresh": "二维码约每 20 秒刷新一次",
  "phone_placeholder": "601234567890（含区号，纯数字）",
  "get_code": "获取配对码",
  "code_placeholder": "点击上方按钮获取配对码",
  "requesting": "正在请求配对码…",
  "fetching": "请求中…",
  "refetch": "重新获取",
  "network_error": "网络错误，请检查连接后重试",
  "how_to_use": "如何使用配对码",
  "step1": "打开手机 WhatsApp",
  "step2": "进入「设置」→「已连接的设备」",
  "step3": "点击「用手机号码连接」",
  "step4": "输入上方显示的 8 位配对码",
  "initializing": "正在初始化，请稍候…",
  "phone_error": "手机号格式错误，请输入含区号的纯数字（如 601234567890）",
  "already_connected": "WhatsApp 已连接，无需配对",
  "not_ready": "WhatsApp 客户端尚未就绪，请等待初始化完成后重试（通常需要 10-20 秒）",
  "status_pending_review": "待 AI 提取",
  "status_ai_extracted": "待发消息",
  "status_confirmed": "已发送",
  "status_rejected": "已拒绝",
  "status_waiting_user_reply": "等待用户回复",
  "ai_extract": "AI 提取",
  "ai_amount": "金额",
  "ai_summary": "摘要",
  "extracting": "识别中…",
  "ai_extract_failed": "AI 提取失败：",
  "network_error_retry": "网络错误，请重试",
  "message_placeholder": "输入要发给用户的消息…",
  "send_to_user": "发送给用户",
  "reject": "拒绝",
  "reject_note": "拒绝原因（可选）",
  "confirm_reject": "确认拒绝此收据？",
  "sent_at": "发送于",
  "rejected_at": "拒绝于",
  "cant_send": "WhatsApp 尚未连接，请先扫码",
  "message_required": "消息内容不能为空",
  "send_not_allowed": "操作不允许：当前状态为 {status}，只有 ai_extracted 状态可发送",
  "download_fail": "下载失败：",
  "load_fail": "加载失败：",
  "password_mismatch": "两次密码输入不一致",
  "login_error": "用户名或密码错误，请重试",
  "login_fail": "登录失败，请重试",
  "receipt_not_found": "收据记录不存在",
  "invalid_status": "当前状态 {status} 不可触发 AI 提取",
  "attempt_limit": "尝试次数过多，请 15 分钟后重试",
  "image_not_found": "图片不存在",
  "ai_recognition_fail": "AI 识别失败：",
  "toast_send_success": "消息已发送",
  "toast_reject_success": "收据已拒绝",
  "toast_ai_success": "AI 提取成功",
  "toast_error": "操作失败",
  "sending": "发送中…",
  "send_fail": "发送失败：",
  "reject_fail": "拒绝失败：",
  "forgot_password": "忘记密码？",
  "contact_admin": "请联系系统管理员重置密码",
  "first_time_setup_link": "首次使用？点击这里初始化",
  "feedback": "反馈",
  "feedback_title": "反馈管理",
  "feedback_new_title": "新建反馈",
  "feedback_detail_title": "反馈详情",
  "feedback_total": "总数",
  "feedback_open": "待处理",
  "feedback_resolved": "已解决",
  "feedback_new": "新建反馈",
  "feedback_search_placeholder": "搜索标题或描述…",
  "feedback_all_status": "全部状态",
  "feedback_status_open": "待处理",
  "feedback_status_resolved": "已解决",
  "feedback_all_type": "全部类型",
  "feedback_type_bug": "缺陷",
  "feedback_type_improvement": "改进建议",
  "feedback_no_feedback": "暂无反馈记录",
  "feedback_title_col": "标题",
  "feedback_type_col": "类型",
  "feedback_status_col": "状态",
  "feedback_submitted_by_col": "提交人",
  "feedback_submitted_at_col": "提交时间",
  "feedback_title_label": "标题",
  "feedback_title_placeholder": "简要描述问题或建议（最多 200 字）",
  "feedback_title_hint": "最多 200 个字符",
  "feedback_type_label": "类型",
  "feedback_type_placeholder": "请选择类型",
  "feedback_description_label": "详细描述",
  "feedback_description_placeholder": "请详细描述问题或建议…",
  "feedback_description_hint": "支持 Markdown 格式",
  "feedback_screenshot_label": "截图",
  "feedback_screenshot_hint": "可选，支持 JPG/PNG/GIF，最大 5MB",
  "feedback_submit": "提交反馈",
  "feedback_submit_success": "反馈已提交成功！GitHub Issue 已创建。",
  "feedback_submit_fail": "提交失败：",
  "feedback_required_fields": "请填写所有必填字段",
  "feedback_title_too_long": "标题不能超过 200 个字符",
  "feedback_description_too_long": "描述不能超过 5000 个字符",
  "feedback_invalid_type": "类型必须是 bug 或 improvement",
  "feedback_file_too_large": "文件大小不能超过 5MB",
  "feedback_upload_fail": "上传失败：",
  "feedback_uploading": "上传中…",
  "feedback_upload_success": "上传成功",
  "feedback_no_file": "未选择文件",
  "feedback_not_found": "反馈记录不存在",
  "feedback_screenshot_not_found": "截图不存在",
  "feedback_no_github_issue": "此反馈尚未关联 GitHub Issue",
  "feedback_sync_status": "同步状态",
  "feedback_sync_confirm": "确定要同步此反馈的状态吗？",
  "feedback_syncing": "同步中…",
  "feedback_sync_success": "状态同步成功！",
  "feedback_sync_fail": "同步失败",
  "feedback_sync_error": "同步出错，请重试",
  "feedback_back_to_list": "返回列表",
  "feedback_view_on_github": "在 GitHub 查看",
  "feedback_github_issue_label": "GitHub Issue",
  "feedback_submitted_by_label": "提交人",
  "feedback_submitted_at_label": "提交时间",
  "feedback_screenshot_alt": "截图"
};
