...
async function apply() {
  ...
  if (!backupPath) throw new Error("数据库文件不存在，未执行迁移");
  console.log("");
  ...
  db.db.prepare("CREATE INDEX IF NOT EXISTS idx_campaigns_active ON campaigns(is_active)").run();
  db.db.prepare("CREATE INDEX IF NOT EXISTS idx_campaigns_dates ON campaigns(start_date, end_date)").run();
